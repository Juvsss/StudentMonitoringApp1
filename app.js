...
function onLogin(){
  ...
  if(currentUser.role === 'Student'){
    ...
    $('dbTab')?.remove();
    $('importCsvBtn')?.remove();
    document.querySelector('[data-tab="dbTab"]')?.remove();
    $('dbTab')?.remove();
...
function updateStudentsTable(filter=''){
  ...
  students.forEach(s=>{
    ...
    if(currentUser.role==='Teacher'){
      attCol = `
     <button data-id="${s.id}" data-status="Present" class="btn success attBtn ${s.attendance.status === 'Present' ? 'active' : ''}">✔Present</button>
    <button data-id="${s.id}" data-status="Absent" class="btn danger attBtn ${s.attendance.status === 'Absent' ? 'active' : ''}">✖Absent</button>
    `;
...
function openAddStudent(){
  ...
  showModal(`<h3>Add Student</h3>
    ...
    <div class="form-row"><label>Class</label><select id="stuClass"><option value=""></option>${clsOptions}</select></div>
